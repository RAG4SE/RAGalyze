...
def get_batch_embedder() -> adal.BatchEmbedder:
    ...
    batch_model_client = batch_model_client_class(
        embedder=model_client, batch_size=embedder_config["batch_size"]
    )
...
def get_code_understanding_client():
    ...
    if "model" not in model_kwargs:
        assert (
            "model" in code_understanding_config
        ), "code_understanding_config must contain model"
