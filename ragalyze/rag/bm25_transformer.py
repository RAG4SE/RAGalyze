...
class BM25Transformer(Component):
    ...
    def build_bm25_index_for_a_file(
        ...
        """Build BM25 tokens for a single file."""
        set_debug_mode(0)
        result = tokenize_for_bm25(code, language, file_path)
        return result
        ...
        try:
            ...
            if self.use_multithreading and len(documents) > 1:
                        ...
                        with tqdm(
                            ...
                            for future in as_completed(future_to_index):
                                ...
                                try:
                                    ...
                                    documents[index].meta_data["bm25_index"] = tokens
                                    pbar.update(1)
                ...
                for doc in tqdm(documents, desc="BM25 single threading indexing"):
                    ...
                    tokens = self.build_bm25_index_for_a_file(
                        text,
                        doc.meta_data["programming_language"],
                        doc.meta_data["file_path"],
                    )
                    doc.meta_data["bm25_index"] = tokens
                end_time = time.time()
                logger.info(
