...
class CodeSplitter(TextSplitter):
    ...
    def __init__(self, 
        ...
        assert file_extension in self.SUFFIX_TO_LANG, f"Unsupported file extension {file_extension}"

        assert split_by == "word" or split_by == "token", "CodeSplitter only supports split_by='word' or split_by='token'"
        ...
        logger.info(f"Initialized CodeSplitter with smart_boundary_ratio={smart_boundary_ratio}, file_extension={file_extension}")
    ...
    def _is_statement_node(self, node, language: str) -> bool:
        ...
        statement_types = {
            ...
            'markdown': {
                'atx_heading', 'setext_heading', 'paragraph', 'code_block',
                'fenced_code_block', 'list_item', 'block_quote',
                'thematic_break', 'html_block', 'link_reference_definition'
            },
            'rst': {
                'section', 'title', 'paragraph', 'literal_block',
                'code_block', 'block_quote', 'bullet_list', 'enumerated_list',
                'list_item', 'definition_list', 'definition_list_item',
                'field_list', 'field', 'option_list', 'option_list_item',
                'doctest_block', 'line_block', 'table', 'directive',
                'comment', 'substitution_definition', 'target', 'transition'
            },
            'yaml': {
                'document', 'block_mapping', 'block_sequence', 'flow_mapping',
                'flow_sequence', 'block_mapping_pair', 'block_sequence_item',
                'flow_mapping_pair', 'flow_sequence_item', 'plain_scalar',
                'single_quote_scalar', 'double_quote_scalar', 'literal_scalar',
                'folded_scalar', 'alias_node', 'anchor_node', 'tag',
                'directive', 'comment'
            },
            'json': {
                'document', 'object', 'array', 'pair', 'string', 'number', 'true', 'false', 'null'
            }
        }
        
        return node.type in statement_types.get(language, set())

    def _find_next_code_boundary_with_treesitter(self, text: bytes, start_pos: int, max_pos: int) -> int:
        """
    ...
    def _find_prev_code_boundary_with_treesitter(self, text: bytes, desired_start_char: int):
        """
        Given a utf-8 encoded text, find the best code boundary using tree-sitter.
        ...
        try:
            ...
            def find_best_start_recursive(node):
                ...
                if node.start_byte >= desired_start_char and self._is_statement_node(node, self.language):
            ...
            logger.error(f"Tree-sitter parsing for overlap failed for {self.language}: {e}")
            raise
    
    def _merge_units_to_chunks_by_words(self, splits: List[str], chunk_size: int, chunk_overlap: int, separator: str):
        chunks = []
        idx = 0
        ...
        while idx < len(splits):
            ...
            prev_chunk_bytes = separator.join(splits[:idx]).encode('utf-8') if idx > 0 else b""
            ...
            if chunk_end < len(splits):
                ...
                smart_boundary_pos = self._find_next_code_boundary_with_treesitter(
                    self.bytes, search_start_pos + len(prev_chunk_bytes), len(chunk_text_bytes) + len(prev_chunk_bytes)
                )
            ...
            if chunk_text_bytes.strip():  # Only add non-empty chunks
                ...
                if last_line_break != -1:
                    chunk_text = (b' ' * (len(prev_chunk_bytes) - last_line_break) + chunk_text_bytes).decode('utf-8')
                else:
            ...
            if num_non_overlap_words > 0 and chunk_end < len(splits):
                bytetext_before_overlap = separator.join(actual_chunk_words[:num_non_overlap_words]).encode('utf-8')
                desired_start_byte = len(bytetext_before_overlap)
                best_start_byte = self._find_prev_code_boundary_with_treesitter(self.bytes, desired_start_byte + len(prev_chunk_bytes))
    ...
    def _merge_units_to_chunks_by_tokens(self, splits: List[str], chunk_size: int, chunk_overlap: int, separator: str) -> List[str]:
        ...
        while idx < len(splits):
            ...
            prev_chunk_bytes = self.tokenizer.decode(splits[:idx]).encode('utf-8') if idx > 0 else b""
    ...
    def _merge_units_to_chunks(self, splits: List[str], chunk_size: int, chunk_overlap: int, separator: str):
        """Enhanced merge method with smart boundary detection."""
        if self.split_by == "word":
            return self._merge_units_to_chunks_by_words(splits, chunk_size, chunk_overlap, separator)
        if self.split_by == "token":
            # For non-token splitting, use the original method
            return self._merge_units_to_chunks_by_tokens(splits, chunk_size, chunk_overlap, separator)
