...
from deepwiki_cli.rag.embedding import (
    ToEmbeddings,
...
class TransformerFactory(ABC):
    ...
    @abstractmethod
    def create_transformer(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> ToEmbeddings:
        """Create transformer instance"""
        pass
    
    @abstractmethod
    def can_handle(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> bool:
...
class DualVectorTransformerFactory(TransformerFactory):
    ...
    def create_transformer(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> DualVectorToEmbeddings:
        code_understanding_generator = kwargs.get('code_understanding_generator')
        if not code_understanding_generator:
            raise ValueError("DualVectorToEmbeddings requires code_understanding_generator")
        
        return DualVectorToEmbeddings(
            embedder=embedder,
            generator=code_understanding_generator
        )
...
class HuggingfaceTransformerFactory(TransformerFactory):
    ...
    def create_transformer(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> HuggingfaceToEmbeddings:
    ...
    def can_handle(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> bool:
        ...
        return embedder_class_name in ["HuggingfaceBatchEmbedder", "HuggingfaceEmbedder"]


class DashScopeTransformerFactory(TransformerFactory):
    """Factory for DashScope transformers"""
    
    def create_transformer(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> DashScopeToEmbeddings:
...
class OpenAITransformerFactory(TransformerFactory):
    ...
    def create_transformer(self, embedder: adal.Embedder | adal.BatchEmbedder, **kwargs) -> OpenAIToEmbeddings:
